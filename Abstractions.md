...
работу многослойной архитектуры, добавив и сразу же выполнив задачу через метод addAndCompleteTask() в UserInterface.
...
В классе UserInterface должен быть реализован публичный метод void addAndCompleteTask(String title), который добавляет задачу и помечает её как выполненную через TaskService.
...
Архитектура позволять заменить реализацию TaskRepository без изменений в TaskService и UserInterface.
...
Если везде используете конкретные классы, а не интерфейсы, изменение реализации потребует переписывать код во многих местах. старайтесь программировать «на уровне интерфейсов».
