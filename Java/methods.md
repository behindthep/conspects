...
toLowerCase() внутри берет строку, преобразует ее и возвращает наружу.
...
random.nextInt(10); // число от 0 до 10
...
```java
Math.random(); // числа, вызов напрямую из класса Math
Files.readString(path); // Чтение данных по пути. Без объекта, статический метод
path.read(); // через объект файла
```


Метод - повысить уровень абстракции, операции, о которых думать как о целом, не погружаясь в детали как работают.
...
#### Перегрузка методов
В Java нельзя задать значение по умолчанию, имитировать с перегрузкой методов. Выбирается метода совпадает по типу и количеству параметров.
