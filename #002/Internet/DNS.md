...
Компьютеру с IP-адресом 192.168.10.36, внутри локальной сети мы указали имя «MIKE-STRATE-PC». командой ping, пошлёт запрос на компьютер ждать от него ответа.

Domain Name System, автоматизировать процессы соотнесения IP-адресов и имён компьютеров, процессы обновления имён у пользователей без ручного скачивания файла со стороннего сервера.

для связи система, запросом в адресной строке подключиться к нужному сервису Записи отвечают за отправку электронных писем, связывают домены и доменные зоны.
