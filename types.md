...
ссылочные типы, то есть объекты: переменная хранит ссылку на место, где объект расположен в памяти.
...
строка ссылочный тип, на каждое значение в коде выделялась доп. память. Без оптимизаций это выражение к двойному выделению памяти. По одной единице памяти на каждый "hm"
```java
"hm" == "hm"; // true
