строить сложные системы, где отдельные части взаимодействуют друг с другом через абстракции (базовые классы или интерфейсы), не заботясь о реализациях.

процесс выделения общих характеристик и поведения группы объектов, частные детали (как что-то работает «под капотом») игнорируются.

Абстрактный класс — у объектов общее состояние, общая логика, дать «скелет» поведения с возможностью доработки.
Интерфейс — только набор методов (контракт), без реализации и состояния. С Java 8 интерфейсы получили default/static-методы, интерфейс — «что должен уметь объект», а не «как он это делает».
...
строить иерархию «от деталей», забывая об общем. странные классы вроде RedCircleWithShadow, которые плохо вписываются в общую структуру. 
сначала выделяйте абстракцию, а потом детали.
...
Абстрактный класс — общее состояние и/или частичная реализация. Интерфейс — только «пообещать» наличие методов, но не хранить данные и не реализовывать поведение.
